...
Cleans Project Gutenberg branding and embeds proper OPF metadata.
...
PG_SUBJECTS_URL = (
    "https://www.gutenberg.org/ebooks/subjects/search/?sort_order=downloads"
)
...
def rewrite_to_mirror(url: str, mirror_base: str) -> str:
    parsed = urlparse(url)
    if (
        "gutenberg.org" in parsed.netloc
        or "pglaf.org" in parsed.netloc
        or "aleph.gutenberg" in parsed.netloc
    ):
...
def parse_opf(
    zipf: zipfile.ZipFile, opf_path: str
...
def add_subjects(md: etree._Element, subjects: List[str]) -> None:
    existing = {
        get_text(el).lower() for el in md.findall("dc:subject", namespaces=NSMAP)
    }
    for s in subjects:
...
def add_collection_tags(
    md: etree._Element, collection_name: str, position: Optional[int]
) -> None:
    """
...
<container version="1.0" xmlns="{NSMAP['container']}">
  <rootfiles>
    ...
    # creators
    existing_creators = [
        get_text(el) for el in md.findall("dc:creator", namespaces=NSMAP)
    ]
    for a in authors:
    ...
    try:
        ...
        for name in z.namelist():
            if name.lower().endswith(
                (".xhtml", ".html", ".htm", ".xml", ".opf", ".ncx", ".txt", ".css")
            ):
...
def run(
    ...
    for gi, subject in enumerate(subjects, 1):
        ...
        for idx, b in enumerate(picked, 1):
            ...
            is_pd_usa = (b.get("copyright") is False) or (
                "Public domain in the USA" in rights
            )
            ...
            if subject not in subjects_list:
                subjects_list = [
                    subject
                ] + subjects_list  # ensure chosen subject present
            ...
            report_rows.append(
                {
                    ...
                    "download_url": dl_url,
                    "series_folder": series_name_used
                    or series_folder_from_meta(title, None),
                    "rights": rights,
                    "status": status,
                    "notes": "; ".join(notes),
                }
    ...
    with (out_reports / "kavita_epub_report.csv").open(
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Download top EPUBs per popular Gutenberg subject, clean + embed Kavita metadata, and output a Kavita-ready library."
    )
    ap.add_argument(
        "--out",
        type=str,
        default="./KavitaLibrary",
        help="Kavita library root (default: ./KavitaLibrary)",
    )
    ap.add_argument(
        "--languages",
        type=str,
    ...
    ap.add_argument(
        ...
        type=str,
        default=MIRROR_BASE,
        help=f"Download mirror base (default: {MIRROR_BASE})",
    )
    ap.add_argument(
        "--sleep",
        type=float,
    ...
    ap.add_argument(
        ...
        type=int,
        default=20,
        help="Titles per subject (default: 20)",
    )
    ap.add_argument(
        "--genres-top",
        type=int,
        default=10,
        help="How many top subjects to auto-scrape (default: 10)",
    )
    ap.add_argument(
        "--genres",
        type=str,
    ...
    ap.add_argument(
        "--no-collections",
        action="store_true",
        help="Do not embed belongs-to-collection metadata",
    )
